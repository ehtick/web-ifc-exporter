import { IFCBUILDING, IFCBUILDINGSTOREY, IFCRELAGGREGATES, IFCRELCONTAINEDINSPATIALSTRUCTURE, IFCSITE, IfcAPI } from 'web-ifc';
import { Mesh } from 'three';
import { ExportHelper } from './ExportHelper';
export class IFCExporter {
    constructor(api = new IfcAPI()) {
        this.api = api;
        this.styleList = [];
        this.uuidList = [];
    }
    async exportMeshAsIFCProduct(exporter, mesh, ifcElement, ifcId, objectPlacement) {
        const brepShapeList = [];
        for (let k = 0; k < mesh.length; k++) {
            ...
            if (mesh[k].material) {
                ...
                if (preexistantMaterial === -1) {
                    ...
                    if (col) {
                        const style = await exporter.ShapePresentationStyleAssignment('material', col.r, col.g, col.b, 1 - opacity);
    ...
    async createModelForExport(projectPlacement, north, objects) {
        if (this.api.wasmModule === undefined)
            await this.api.Init();
        ...
        const project = await exporter.Project(context, 'web-ifc-three', 'this project was exported from web-ifc-three');
        const elementsList = [];
        ...
        SiteList.push(site);
        await exporter.RelContainedInSpatialStructure(IFCRELCONTAINEDINSPATIALSTRUCTURE, buildingStorey, elementsList);
